const app = {
    ...
    bindAddProductsEvents: function () {
        ...
        const renderResults = (results) => {
            ...
            // Make entire product container clickable by adding event to container
            const productItems = resultsList.querySelectorAll('.add-product-item');
            productItems.forEach(item => {
                item.addEventListener('click', (e) => {
                    // Find checkbox within this item
                ...
                <li class="splide__slide">
                    <div class="celeb-card">
                        <div class="celeb-avatar-wrapper">
                            <img src="${c.avatar}" alt="${c.nickname}" class="celeb-avatar">
                            ${c.isVerified ? '<div class="celeb-badge"><svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg></div>' : ''}
                        </div>
                        <div class="celeb-info">
                            <div class="celeb-nickname">@${c.nickname}</div>
                            <div class="celeb-fullname">${fullname}</div>
                        </div>
                        ...
                        <a href="#" class="celeb-btn-view">Products (${Math.floor(Math.random() * (20 - 3 + 1)) + 3})</a>
                    </div>
                </li>
            `;
            list.insertAdjacentHTML('beforeend', html);
            ...
            if ((isSearchPage && index === 0) || !isSearchPage) {
                statusIcon = `
                 <svg viewBox="0 0 24 24" width="24" height="24" class="icon-favorite product-card-favorite">
                    <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" fill="#b60378" />
                 </svg>`;
            }
                ...
                <div class="product-right-col">
                    <div>
                        ...
                        <ul class="product-actions">
                            ...
                            ${p.gtin ? `<li class="compare-prices-link" data-gtin="${p.gtin}" data-product-id="${p.id}">
    ...
    bindComparePricesEvents: function () {
        ...
        function renderResults() {
            ...
            filtered.forEach((p) => {
                try {
                    // Convert to string first in case price is a number
                    const priceStr = String(p.price || '');
                    const priceVal = parseFloat(priceStr.replace(/,/g, '').replace(/[^\d.]/g, ''));
            ...
            filtered.forEach((p) => {
                ...
                try {
                    ...
                    console.log('Product:', p.title, '| Price string:', priceStr, '| Parsed:', priceVal, '| Lowest:', lowestPrice, '| Is lowest?', isLowestPrice);
                } catch (e) {
